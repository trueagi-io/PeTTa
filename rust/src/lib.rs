...
fn build_server_source(src_dir: &Path, verbose: bool) -> Result<String, PeTTaError> {
    ...
    for file in &["parser.pl", "spaces.pl", "specializer.pl", "translator.pl", "filereader.pl", "metta.pl"] {
        let fpath = src_dir.join(file);
        if !fpath.exists() { return Err(PeTTaError::FileNotFound(fpath)); }
...
write_response(Results) :-
    ...
    put_byte(user_output, 0),  % status byte: 0 = success
    write_u32(Count),
    maplist(write_result_str, Results).

write_error_response(Error) :-
    put_byte(user_output, 1),  % status byte: 1 = error
...
pub enum MettaValue {
    Integer(String), Float(f64), Bool(bool), Atom(String),
    List(Vec<MettaValue>), Expression(String, Vec<MettaValue>),
}

impl MettaValue {
    pub fn parse(s: &str) -> Option<Self> { parse_sexpr(s.trim()) }
}
    ...
    if let Some(inner) = strip_parens(s) {
        ...
        if parsed.len() == items.len() && !parsed.is_empty() {
            if let MettaValue::Atom(head) = &parsed[0] {
                if parsed.len() > 1 { return Some(MettaValue::Expression(head.clone(), parsed[1..].to_vec())); }
            }
...
fn strip_parens(s: &str) -> Option<&str> {
    if s.starts_with('(') && s.ends_with(')') && is_balanced(&s[1..s.len()-1]) { Some(&s[1..s.len()-1]) } else { None }
}

fn is_balanced(s: &str) -> bool {
    let mut d = 0i32;
    for c in s.chars() { match c { '(' => d+=1, ')' => { d-=1; if d<0 { return false; } }, _ => {} } }
    d == 0
}

fn split_top_level(s: &str) -> Vec<String> {
    let mut items = Vec::new(); let mut cur = String::new();
    let (mut depth, mut in_q, mut esc) = (0i32, false, false);
    for ch in s.chars() {
        if esc { cur.push(ch); esc = false; continue; }
        if ch == '\\' { esc = true; cur.push(ch); continue; }
        if ch == '"' { in_q = !in_q; cur.push(ch); continue; }
        if in_q { cur.push(ch); continue; }
        match ch {
            '(' => { depth += 1; cur.push(ch); }
            ')' => { depth -= 1; cur.push(ch); }
            ' ' | '\t' | '\n' | '\r' if depth == 0 => { if !cur.is_empty() { items.push(std::mem::take(&mut cur)); } }
            _ => cur.push(ch),
        }
    }
...
pub struct MettaResult { pub value: String }
...
impl std::fmt::Display for SwiplErrorKind {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            SwiplErrorKind::UndefinedFunction { name, arity } => write!(f, "MeTTa function '{}/{}' is not defined", name, arity),
            SwiplErrorKind::TypeMismatch { expected, found } => write!(f, "Type error: expected {}, got {}", expected, found),
            SwiplErrorKind::SyntaxError { detail } => write!(f, "Syntax error: {}", detail),
            SwiplErrorKind::UninstantiatedArgument => write!(f, "Argument is not sufficiently instantiated"),
            SwiplErrorKind::PermissionDenied { operation, target } => write!(f, "Permission denied: {} on {}", operation, target),
            SwiplErrorKind::StackOverflow => write!(f, "Stack overflow"),
...
fn parse_swipl_error(raw: &str) -> SwiplErrorKind {
    if raw.contains("existence_error") && raw.contains("procedure") {
        if let Some(c) = extract_name_arity(raw) { return SwiplErrorKind::UndefinedFunction { name: c.0, arity: c.1 }; }
    }
    if raw.contains("type_error") { return SwiplErrorKind::TypeMismatch { expected: "unknown".into(), found: "unknown".into() }; }
    if raw.contains("syntax_error") { return SwiplErrorKind::SyntaxError { detail: extract_between(raw, "syntax_error(", ")").unwrap_or_else(|| "unknown".into()) }; }
...
fn extract_name_arity(raw: &str) -> Option<(String, usize)> {
    for t in raw.split(&['(',')',',','/']) {
        let t = t.trim(); let p: Vec<&str> = t.split_whitespace().collect();
        if p.len() == 2 { if let Ok(a) = p[1].parse::<usize>() { return Some((p[0].to_string(), a)); } }
    }
    None
}

fn extract_between(s: &str, start: &str, end: &str) -> Option<String> {
    let si = s.find(start)?; let rest = &s[si + start.len()..];
    let ei = rest.find(end)?; Some(rest[..ei].to_string())
}
...
#[derive(Debug)]
pub enum PeTTaError {
    FileNotFound(PathBuf), SpawnSwipl(String), PathError(String), WriteError(String),
    SwiplError(SwiplErrorKind), SwiplVersionError(String), ProtocolError(String),
}
...
impl PeTTaEngine {
    pub fn new(project_root: &Path, verbose: bool) -> Result<Self, PeTTaError> {
        ...
        let tmp = tempfile::Builder::new().prefix("petta_srv_").suffix(".pl").tempfile()
        ...
        let mut child = Command::new("swipl")
            .arg("-q").arg("-t").arg("halt")
            .arg(&tmp_path)
            .stdin(Stdio::piped()).stdout(Stdio::piped()).stderr(Stdio::piped())
            .spawn().map_err(|e| PeTTaError::SpawnSwipl(e.to_string()))?;

        // Consume stderr to prevent blocking
        let stderr = child.stderr.take();
        std::thread::spawn(move || {
            if let Some(mut s) = stderr { let mut buf = [0u8; 4096]; while let Ok(n) = s.read(&mut buf) { if n == 0 { break; } } }
        });
        ...
        let mut engine = Self { child: Some(child), stdin_pipe: Some(stdin), stdout_pipe: Some(stdout) };
    ...
    fn wait_for_ready(&mut self) -> Result<(), PeTTaError> {
        loop {
            let mut b = [0u8; 1];
            self.stdout_pipe.as_mut().unwrap().read_exact(&mut b).map_err(|e| {
    ...
    fn read_u32(&mut self) -> Result<u32, PeTTaError> {
        let mut b = [0u8; 4];
        self.stdout_pipe.as_mut().unwrap().read_exact(&mut b).map_err(|e| {
            if e.kind() == std::io::ErrorKind::UnexpectedEof { PeTTaError::ProtocolError("child closed".into()) }
            else { PeTTaError::ProtocolError(e.to_string()) }
        })?;
        Ok(u32::from_be_bytes(b))
    }

    fn send_query(&mut self, query_type: u8, payload: &str) -> Result<Vec<MettaResult>, PeTTaError> {
        let pb = payload.as_bytes();
        let len = pb.len() as u32;
        let sin = self.stdin_pipe.as_mut().unwrap();
        sin.write_all(&[query_type]).map_err(|e| PeTTaError::WriteError(e.to_string()))?;
        sin.write_all(&len.to_be_bytes()).map_err(|e| PeTTaError::WriteError(e.to_string()))?;
        sin.write_all(pb).map_err(|e| PeTTaError::WriteError(e.to_string()))?;
        sin.flush().map_err(|e| PeTTaError::WriteError(e.to_string()))?;

        let status = { let mut b = [0u8; 1];
            self.stdout_pipe.as_mut().unwrap().read_exact(&mut b)
                .map_err(|e| PeTTaError::ProtocolError(e.to_string()))?; b[0] };
        ...
        match status {
            0 => {
                ...
                for _ in 0..count {
                    ...
                    self.stdout_pipe.as_mut().unwrap().read_exact(&mut buf)
    ...
    pub fn shutdown(&mut self) {
        if let Some(sin) = self.stdin_pipe.as_mut() { let _ = sin.write_all(&[b'Q', 0, 0, 0, 0]); let _ = sin.flush(); }
        if let Some(mut child) = self.child.take() { let _ = child.wait(); }
    }
}

impl Drop for PeTTaEngine { fn drop(&mut self) { self.shutdown(); } }
...
fn parse_output(output: &str, _verbose: bool) -> Vec<MettaResult> {
    output.lines().filter_map(|line| {
        ...
        if cleaned.is_empty() || cleaned.starts_with('%') || cleaned.starts_with("?-")
            ...
            || cleaned.starts_with('!') || cleaned.contains("metta function")
            || cleaned.contains("metta runnable") || cleaned.contains("prolog clause")
            || cleaned.contains("prolog goal") { return None; }
        Some(MettaResult { value: cleaned })
    }).collect()
}

fn strip_ansi(s: &str) -> String {
    let mut result = String::new(); let mut in_esc = false;
    for ch in s.chars() {
        if ch == '\x1b' { in_esc = true; }
        else if in_esc { if ch == '[' { continue; } if (0x40..=0x7E).contains(&(ch as u32)) { in_esc = false; } }
        else { result.push(ch); }
    }
...
const MIN_MAJOR: u32 = 9; const MIN_MINOR: u32 = 3;

fn check_swipl_version() -> Result<(), PeTTaError> {
    let output = Command::new("swipl").arg("--version").output().map_err(|_| {
        PeTTaError::SwiplVersionError(format!("swipl not found. Install SWI-Prolog >= {}.{}.", MIN_MAJOR, MIN_MINOR))
    })?;
    if !output.status.success() { return Err(PeTTaError::SwiplVersionError("swipl --version failed".into())); }
    ...
    for part in vs.split_whitespace() {
        ...
        if p.len() >= 2 {
            if let (Ok(ma), Ok(mi)) = (p[0].parse::<u32>(), p[1].parse::<u32>()) {
                if ma > MIN_MAJOR || (ma == MIN_MAJOR && mi >= MIN_MINOR) { return Ok(()); }
                if ma < MIN_MAJOR { return Err(PeTTaError::SwiplVersionError(format!("SWI-Prolog {}.{} found, need >= {}.{}", ma, mi, MIN_MAJOR, MIN_MINOR))); }
            }
        }
    }
    Ok(())
...
mod tests {
    ...
    fn make_engine() -> PeTTaEngine { PeTTaEngine::new(&project_root(), false).expect("Failed to create engine") }

    #[test] fn test_engine_creation() { let r = PeTTaEngine::new(&project_root(), false); if let Err(ref e) = r { eprintln!("Engine creation failed: {}", e); } assert!(r.is_ok()); }
    #[test] fn test_identity() { let mut e = make_engine(); let r = e.process_metta_string("(= (myid $x) $x) !(myid 42)").unwrap(); assert!(!r.is_empty()); assert_eq!(r[0].value, "42"); }
    #[test] fn test_arithmetic() { let mut e = make_engine(); let r = e.process_metta_string("!(+ 1 2)").unwrap(); assert!(!r.is_empty()); assert_eq!(r[0].value, "3"); }
    #[test] fn test_load_identity_file() { let mut e = make_engine(); let r = e.load_metta_file(&project_root().join("examples/identity.metta")).unwrap(); assert!(!r.is_empty()); }
    #[test] fn test_boolean() { let mut e = make_engine(); let r = e.process_metta_string("!(and true false)").unwrap(); assert!(!r.is_empty()); assert_eq!(r[0].value, "false"); }
    #[test] fn test_comparison() { let mut e = make_engine(); let r = e.process_metta_string("!(< 1 2)").unwrap(); assert!(!r.is_empty()); assert_eq!(r[0].value, "true"); }
    #[test] fn test_fibonacci() { let mut e = make_engine(); let r = e.load_metta_file(&project_root().join("examples/fib.metta")).unwrap(); assert!(!r.is_empty()); }
    #[test] fn test_state() { let mut e = make_engine(); let r = e.load_metta_file(&project_root().join("examples/state.metta")).unwrap(); assert!(!r.is_empty()); }
    #[test] fn test_if() { let mut e = make_engine(); let r = e.load_metta_file(&project_root().join("examples/if.metta")).unwrap(); assert!(!r.is_empty()); }
    #[test] fn test_math() { let mut e = make_engine(); let r = e.load_metta_file(&project_root().join("examples/math.metta")).unwrap(); assert!(!r.is_empty()); }
    #[test] fn test_variable_renaming() { let mut e = make_engine(); let r = e.process_metta_string("(= (fun ($a x)) ($b x)) !(fun (a x))").unwrap(); assert!(!r.is_empty()); let v = &r[0].value; assert!(v.contains("$") && v.contains('x'), "Expected variable pattern, got: {}", v); }
    #[test] fn test_file_imports() { let mut e = make_engine(); let root = project_root(); let id = std::fs::read_to_string(root.join("examples/identity.metta")).unwrap(); let r = e.process_metta_string(&format!("{}\n!(f 5)", id)).unwrap(); assert!(r.iter().any(|x| x.value == "25"), "Expected '25': {:?}", r); }
    #[test] fn test_verbose_mode() { let root = project_root(); let mut e = PeTTaEngine::new(&root, true).unwrap(); let r = e.process_metta_string("!(+ 1 2)").unwrap(); assert!(!r.is_empty()); assert_eq!(r[0].value, "3"); }
    #[test] fn test_parse_output() { let r = parse_output("42\n(a b c)\nhello", false); assert_eq!(r.len(), 3); assert_eq!(r[0].value, "42"); }
    #[test] fn test_parse_output_filters_debug() { let r = parse_output("--> metta function -->\n42\n^^^^^^^^^^^^^^^^^^^\n\x1b[36m!(+ 1 2)\n\x1b[33m-->", false); assert_eq!(r.len(), 1); assert_eq!(r[0].value, "42"); }
    #[test] fn test_parse_int() { assert_eq!(MettaValue::parse("42"), Some(MettaValue::Integer("42".into()))); }
    #[test] fn test_parse_bool() { assert_eq!(MettaValue::parse("true"), Some(MettaValue::Bool(true))); assert_eq!(MettaValue::parse("false"), Some(MettaValue::Bool(false))); }
    ...
    #[test] fn test_parse_list() { let v = MettaValue::parse("(1 2 3)"); assert!(matches!(v, Some(MettaValue::List(ref items)) if items.len() == 3)); }
    #[test] fn test_parse_expression() { let v = MettaValue::parse("(+ 1 2)"); assert!(matches!(v, Some(MettaValue::Expression(ref f, ref args)) if f == "+" && args.len() == 2)); }
    #[test] fn test_parse_undefined_function_error() { assert!(matches!(parse_swipl_error("ERROR: existence_error(procedure, (/ foo 2))"), SwiplErrorKind::UndefinedFunction { .. })); }
    #[test] fn test_parse_stack_overflow() { assert!(matches!(parse_swipl_error("ERROR: Stack depth: 5000000"), SwiplErrorKind::StackOverflow)); }
    #[test] fn test_result_parsed_value() { let r = MettaResult { value: "42".into() }; assert_eq!(r.parsed_value(), Some(MettaValue::Integer("42".into()))); }
}

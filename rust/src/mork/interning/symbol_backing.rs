...
impl Slab {
  pub(crate) unsafe fn allocate(bytes : u64)-> *mut Slab {
    ...
    let layout = std::alloc::Layout::array::<core::cell::UnsafeCell<u8>>(slab_size).unwrap().align_to(4096).expect("Cannot be aligned");
  ...
  pub(crate) unsafe fn free(mut slab : *mut Self) {
    while !slab.is_null() {
      unsafe {
        ...
        std::alloc::dealloc(cur as *mut u8, std::alloc::Layout::array::<u8>(size).unwrap().align_to(4096).unwrap());
      }
    }
  }

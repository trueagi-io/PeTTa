...
mod test {
    ...
    fn serialize_long() {
      ...
      "; ARR_LEN];
        ...
        load.to_bytes[0].0.read().unwrap().read_zipper().into_cata_side_effect(
            |_mask, _accs, val: Option<&ThinBytes>, _path| {
                match val {
                    Some(id) => core::assert!(
    ...
    unsafe fn as_btree(
        ...
        for each in 0..MAX_WRITER_THREADS {
            ...
            for (value, path) in shared_mapping.to_bytes[each].0.read().unwrap().iter() {
                ...
                out2.insert(unsafe { *(value.as_ptr() as *const [u8; SYM_LEN]) }, unsafe {
                    core::mem::transmute((&*path.as_raw_slice()).to_owned())
                });
            }
        }

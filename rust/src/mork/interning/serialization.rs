...
use crate::pathmap::{morphisms::Catamorphism, PathMap};
use zip::ZipArchive;
use super::{bounded_pearson_hash, SharedMapping, SharedMappingHandle, Slab, Symbol, ThinBytes, MAX_WRITER_THREADS, SYMBOL_THREAD_PERMIT_BYTE_POS, SYM_LEN, PEARSON_BOUND};
    ...
    for file_name in files {
      ...
      unsafe {
        ...
        while let Some((sym_bytes, to_parse_0)) = to_parse.split_at_checked(SYM_LEN-SYMBOL_THREAD_PERMIT_BYTE_POS) {
          ...
          to_symbol[bounded_pearson_hash::<{PEARSON_BOUND}>(&to_parse[0..length]) as usize % MAX_WRITER_THREADS].insert(&to_parse_2[0..length], sym);         

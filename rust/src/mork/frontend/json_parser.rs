...
impl<'a> Parser<'a> {
    ...
    fn unexpected_character<T: Sized>(&mut self) -> Result<T> {
        ...
        let ch = self.source[at..].chars().next().expect("Must have a character");

        let (lineno, col) = self.source[..at].lines().enumerate().last().unwrap_or((0, ""));

        let colno = col.chars().count();

        Err(Error::UnexpectedCharacter { ch: ch, line: lineno + 1, column: colno + 1 })
    }
    ...
    fn read_big_number(&mut self, num: &mut u64, e: &mut i16) -> Result<()> {
        loop {
            ...
            match ch {
                b'0'..=b'9' => {
                    self.bump();
                    match num.checked_mul(10).and_then(|num| num.checked_add((ch - b'0') as u64)) {
            ...
            'parsing: loop {
                match ch {
                    ...
                    b'{' => {
                        ...
                        for s in t.write_empty_object() {
                            yield s
                        }
                    }
                    b'"' => {
                        let s = expect_string!(self);
                        for si in t.write_string(s) {
                            yield si
                        }
                    }
                    b'0' => {
                        ...
                        for si in t.write_number(false, mantissa, exponent) {
                            yield si
                        }
                    }
                    b'1'..=b'9' => {
                        ...
                        let mut exponent = 0;
                        expect_number!(self, _mantissa, exponent, ch);
                        for si in t.write_number(false, _mantissa, exponent) {
                            yield si
                        }
                    }
                    b'-' => {
                        ...
                        match ch {
                            b'0' => {
                                ...
                                for si in t.write_number(true, mantissa, exponent) {
                                    yield si
                                }
                            }
                            b'1'..=b'9' => {
                                ...
                                let mut exponent = 0;
                                expect_number!(self, _mantissa, exponent, ch);
                                for si in t.write_number(true, _mantissa, exponent) {
                                    yield si
                                }
                            }
                            _ => return self.unexpected_character(),
                    ...
                    b't' => {
                        expect_sequence!(self, b'r', b'u', b'e');
                        for si in t.write_true() {
                            yield si
                        }
                    }
                    b'f' => {
                        expect_sequence!(self, b'a', b'l', b's', b'e');
                        for si in t.write_false() {
                            yield si
                        }
                    }
                    b'n' => {
                        expect_sequence!(self, b'u', b'l', b'l');
                        for si in t.write_null() {
                            yield si
                        }
                    }
                    _ => return self.unexpected_character(),

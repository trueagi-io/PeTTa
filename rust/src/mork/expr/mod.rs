        ...
        let value = 'putting: loop {
            ...
            'popping: loop {
                match stack.last_mut() {
                    ...
                    Some(&mut State { iter: ref mut k, payload: ref mut acc }) => unsafe {
...
impl Expr {
    ...
    pub fn arity(self) -> Option<u8> {
        unsafe { if let Tag::Arity(n) = byte_item(*self.ptr) { Some(n) } else { None } }
    }
    ...
    pub fn transformed(self, template: Expr, pattern: Expr) -> Result<Expr, ExtractFailure> {
        ...
        if let Ok(bindings) = unify(s) {
            ...
            apply(
                ...
                0,
                0,
                &mut ExprZipper::new(Expr { ptr: transformation.as_mut_ptr() }),
                &bindings,
    ...
    pub fn serialize<Target: std::io::Write, F: for<'a> Fn(&'a [u8]) -> &'a str>(
        ...
        let mut traversal =
            SerializerTraversal { out: t, map_symbol: map_symbol, transient: false };
        execute_loop(&mut traversal, *self, 0);
    }
    ...
    for PairTraversal<A1, A2, R1, R2, T1, T2>
{
    ...
    fn add(&mut self, offset: usize, acc: (A1, A2), sub: (R1, R2)) -> (A1, A2) {
        (self.t1.add(offset, acc.0, sub.0), self.t2.add(offset, acc.1, sub.1))
    }
    fn finalize(&mut self, offset: usize, acc: (A1, A2)) -> (R1, R2) {
        (self.t1.finalize(offset, acc.0), self.t2.finalize(offset, acc.1))
    }
}
            ...
            Some(&mut Breadcrumb { parent: _p, arity: a, seen: ref mut s }) => {
                ...
                if *s < a {
                    *s += 1;
                    ...
                    self.loc +=
                        if let Tag::SymbolSize(n) = self.tag() { n as usize + 1 } else { 1 };
    ...
    pub fn args(&self, dest: &mut Vec<Self>) {
        unsafe {
            match byte_item(*self.subsexpr().ptr) {
                ...
                Tag::Arity(k) => {
                    let mut env =
                        ExprEnv { n: self.n, v: self.v, offset: self.offset + 1, base: self.base };
                    for sk in 0..k {
...
pub fn unify(
    ...
    if stack.is_empty() { Ok(bindings) } else { unreachable!() }
}

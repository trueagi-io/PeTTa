...
impl MORKEngine {
    pub fn new() -> Self {
        ...
        MORKEngine { space: space_arc.clone(), interpreter: Interpreter::new(space_arc) }
    }
    ...
    fn process_line(&mut self, line: &str) -> String {
        ...
        match cmd {
            "add-atoms" => self
                .add_atoms(parts.get(1).unwrap_or(&""))
                .unwrap_or_else(|e| format!("ERR: {}", e)),
            "remove-atoms" => self
                .remove_atoms(parts.get(1).unwrap_or(&""))
                .unwrap_or_else(|e| format!("ERR: {}", e)),
            "get-atoms" => self.get_atoms().unwrap_or_else(|e| format!("ERR: {}", e)),
            "match" => self
                .match_pattern(parts.get(1).unwrap_or(&""))
                .unwrap_or_else(|e| format!("ERR: {}", e)),
            "mm2-exec" => {
                self.mm2_exec(parts.get(1).unwrap_or(&"")).unwrap_or_else(|e| format!("ERR: {}", e))
            }
            _ => {

...
impl<V: Clone> Clone for BridgeNode<V> {
    fn clone(&self) -> Self {
        Self { header: self.header, key_bytes: self.key_bytes, payload: self.clone_payload() }
    }
}
...
impl<V: Clone + Send + Sync> BridgeNode<V> {
        ...
        V: Lattice,
    {
        ...
        if overlap > 0 {
            ...
            if overlap > 0 {
                let new_node =
                    Self::new(&self_key[..overlap], true, TrieNodeODRc::new(split_node).into());
                return TrieNodeODRc::new(new_node);
            } else {
...
impl<V: Clone + Send + Sync> TrieNode<V> for BridgeNode<V> {
    fn node_contains_partial_key(&self, key: &[u8]) -> bool {
        debug_assert!(!self.is_empty());
        if self.key().starts_with(key) { true } else { false }
    }
...
fn test_bridge_node() {
    ...
    let (consumed_bytes, child_node) =
        new_node.node_get_child(b"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz").unwrap();

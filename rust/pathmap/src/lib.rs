...
pub mod gxhash {
    ...
    #[cfg(feature = "fast-hasher")]
    pub use fast::{GxHashMap, GxHasher, HashMapExt};
    
    #[cfg(feature = "fast-hasher")]
    pub fn with_seed(seed: u64) -> fast::GxHasher {
        fast::GxHasher::default()
    }
    ...
    mod fallback {
        ...
        impl core::hash::Hasher for GxHasher {
            ...
            fn write(&mut self, bytes: &[u8]) {
                self.0 = xxh3_64(bytes);
            }
        }
        
        impl core::hash::BuildHasher for GxHasher {
            type Hasher = Self;
            
            fn build_hasher(&self) -> Self::Hasher {
                *self
            }
            fn build_hasher_default() -> Self {
                Self(0)
            }
        }
        
        pub type HashMap<K, V> = std::collections::HashMap<K, V, GxHasher>;
        ...
        impl<K, V> HashMapExt<K, V> for HashMap<K, V> {
            fn new() -> Self {
                std::collections::HashMap::with_hasher(GxHasher::default())
            }
            fn with_capacity(cap: usize) -> Self {
                std::collections::HashMap::with_capacity_and_hasher(cap, GxHasher::default())
            }
        }
    }
    
    #[cfg(not(feature = "fast-hasher"))]
    pub use fallback::{GxHasher, HashMap, HashMapExt, with_seed};
...
pub mod pathmap {
    //! Re-export for macro compatibility
    pub use crate::{ring, trie_core, zipper, morphisms, merkleization, utils, experimental, alloc, PathMap, TrieValue};
    pub mod gxhash {

...
:- dynamic metta_memo_mode/3.
:- dynamic metta_memo_probe/4.

metta_memo_limit(10000).
metta_memo_probe_window(32).
...
invalidate_metta_memo_fun(Fun) :-
    findall(Arity, arity(Fun, Arity), RawArities),
    sort(RawArities, Arities),
    ( Arities == [] -> true
    ; forall(member(Arity, Arities),
             ( bump_metta_memo_generation(Fun, Arity),
               retractall(metta_memo_mode(Fun, Arity, _)),
               retractall(metta_memo_probe(Fun, Arity, _, _))
             ))
    ).

clear_metta_memo_cache :-
    retractall(metta_memo_entry(_, _, _, _, _, _)),
    retractall(metta_memo_generation(_, _, _)),
    retractall(metta_memo_mode(_, _, _)),
    retractall(metta_memo_probe(_, _, _, _)),
...
memo_mode(_, _, enabled).

memo_observe_unknown(_, _, _).

memoized_fun_call(Fun, AVs, Out) :-
